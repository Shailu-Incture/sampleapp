jQuery.sap.declare("com.murphy.ioprocapp.util.formatter");

com.murphy.ioprocapp.util.formatter = {
	addMiles: function(prop) {
		if (prop !== null) {
			return prop + " miles";
		}
	},
	...
	navBackButtonEnable: function(sProp) {
		var _bEnabled = true;
		if (sProp === "FIELD" || sProp === null || sProp === undefined || sProp === "") {
			_bEnabled = false;
		}
		return _bEnabled;
	},
	...
	fnShowTaskDetails: function(oValue, oMsg) {
		if (oValue === "Non-Dispatch") {
			return false;
		} else if ((oMsg !== undefined) && (oMsg === null || oMsg === "")) {
			return false;
		} else {
			return true;
		}
	...
	getLocalDate: function(oDate, oString, oCreate) {
		var returnString = "";
		if (oString) {
	...
	getAlarmColor: function(oStatus) {
